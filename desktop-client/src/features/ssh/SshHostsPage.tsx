...
export function SshHostsPage({ onSession }: SshHostsPageProps) {
  ...
  const selected = hosts.find((host) => host.id === selectedId) || filtered[0] || null;

  const run = async (label: string, action: () => Promise<unknown>) => {
    setBusy(label); setError(""); setNotice("");
    try { await action(); setNotice(label === "test" ? "免密测试已完成" : "操作已完成"); await load(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(""); }
  };
  ...
  const connect = () => selected ? void run("connect", async () => { onSession(await window.afkDesktop.ssh.connect(selected.id)); }) : undefined;
  ...
  return <section className="control-page ssh-page" aria-label="SSH 主机管理">
    ...
    <section className="ssh-toolbar"><label className="ssh-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索别名、地址或用户" /></label><select value={source} onChange={(event) => setSource(event.target.value as SourceFilter)} aria-label="来源筛选"><option value="all">全部来源</option><option value="system">系统配置</option><option value="managed">AFK 管理</option></select><select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} aria-label="状态筛选"><option value="all">全部状态</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button className="icon-button" onClick={() => void load()} disabled={!!busy} aria-label="刷新 SSH 主机"><RefreshCw size={16} className={busy === "list" ? "spin" : ""} /></button></section>
    ...
    {formOpen ? <div className="ssh-modal-backdrop"><form className="ssh-modal" onSubmit={(event) => { event.preventDefault(); void add(); }}><header><div><small>AFK 管理</small><h2>添加 SSH 主机</h2></div><button type="button" className="icon-button" onClick={() => setFormOpen(false)} aria-label="关闭"><X size={16} /></button></header><label>Host 别名<input required value={form.alias} onChange={(event) => setForm({ ...form, alias: event.target.value })} placeholder="production-web" /></label><label>主机地址<input required value={form.hostname} onChange={(event) => setForm({ ...form, hostname: event.target.value })} placeholder="203.0.113.10" /></label><div className="ssh-form-row"><label>端口<input type="number" min="1" max="65535" value={form.port} onChange={(event) => setForm({ ...form, port: Number(event.target.value) })} /></label><label>用户<input value={form.user || ""} onChange={(event) => setForm({ ...form, user: event.target.value })} placeholder="deploy" /></label></div><label>已有私钥路径（可选）<input value={form.identityFile || ""} onChange={(event) => setForm({ ...form, identityFile: event.target.value || undefined })} placeholder="~/.ssh/id_ed25519" /></label><label>跳板机（可选）<input value={form.proxyJump || ""} onChange={(event) => setForm({ ...form, proxyJump: event.target.value || undefined })} placeholder="bastion" /></label><footer><button type="button" className="secondary-action" onClick={() => setFormOpen(false)}>取消</button><button type="submit" className="primary-action" disabled={busy === "add"}>{busy === "add" ? "保存中…" : "保存主机"}</button></footer></form></div> : null}
  </section>;
}

function SshDetails({ host, busy, onTrust, onTest, onConnect, onDeploy, onRemove }: { host: SshHost | null; busy: string; onTrust: () => void; onTest: () => void; onConnect: () => void; onDeploy: () => void; onRemove: () => void }) {
  ...
  return <aside className="ssh-details" aria-label={`${host.alias} SSH 详情`}><header><div><small>{sourceLabels[host.source]}</small><h2>{host.alias}</h2><span>{host.user ? `${host.user}@` : ""}{host.hostname}:{host.port}</span></div><span className={`ssh-status ${host.status}`}>{statusLabels[host.status]}</span></header><dl><div><dt>配置来源</dt><dd>{host.configPath}</dd></div><div><dt>HostName</dt><dd>{host.hostname}</dd></div><div><dt>IdentityFile</dt><dd>{host.identityFile || "系统默认密钥"}</dd></div><div><dt>ProxyJump</dt><dd>{host.proxyJump || "无"}</dd></div>{host.fingerprint ? <div><dt>主机指纹</dt><dd className="fingerprint">{host.fingerprint.algorithm} · {host.fingerprint.value}</dd></div> : null}</dl><div className="ssh-actions">{host.status === "untrusted" && host.fingerprint ? <button className="primary-action" onClick={onTrust} disabled={!!busy}><ShieldCheck size={15} />信任此指纹</button> : null}{host.status === "auth-required" ? <button className="secondary-action" onClick={onDeploy} disabled={!!busy}><Copy size={15} />部署公钥</button> : null}<button className="secondary-action" onClick={onTest} disabled={!!busy || blocked || host.status === "untrusted"}><Link2 size={15} />测试免密</button><button className="primary-action" onClick={onConnect} disabled={!!busy || blocked || host.status !== "ready"}><Server size={15} />连接</button>{host.source === "managed" ? <button className="danger-action" onClick={onRemove} disabled={!!busy}><Trash2 size={15} />删除</button> : null}</div><p className="ssh-safety-note"><ShieldCheck size={14} />私钥、密码和终端输入由系统 OpenSSH 处理，AFK 不会保存。</p></aside>;
}
