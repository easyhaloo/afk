...
export function GraphReviewPanel({ workspace, templateId = 'sequential-review' }: { workspace: string; templateId?: string }) {
  ...
  const refresh = async () => {
    if (!workspace) return;
    setStatus(await window.afkDesktop.graphStatus(workspace, selectedTemplate) as GraphStatus);
  };
  const generate = async (format: 'json' | 'archify-json' = 'json') => {
    if (!workspace) return;
    setBusy(true);
    try { setStatus((await window.afkDesktop.graphGenerate({ workspace, templateId: selectedTemplate, format })).status as GraphStatus); }
    finally { setBusy(false); }
  };
  useEffect(() => { void refresh(); }, [workspace, selectedTemplate]);
  return <section className="graph-review-panel" aria-label="工作流审阅图"><header><div><span className="graph-eyebrow"><Workflow size={13} />WORKFLOW GRAPH</span><h2>审阅图</h2><label className="graph-template">模板 ID<input value={selectedTemplate} onChange={(event) => setSelectedTemplate(event.target.value.toLowerCase())} onKeyDown={(event) => { if (event.key === 'Enter') void refresh(); }} /></label></div><div className="graph-actions"><button onClick={() => void refresh()} title="重新读取"><RefreshCw size={14} /></button><button onClick={() => void generate('archify-json')} title="导出 Archify JSON"><Download size={14} /></button><button className="graph-primary" disabled={busy || !selectedTemplate} onClick={() => void generate()}>{busy ? '生成中…' : '生成审阅图'}</button></div></header><div className={`graph-status graph-status-${status.state}`}><i />{status.state === 'trusted' ? '已信任' : status.state === 'missing' ? '尚未生成' : status.state === 'stale' ? '输入已变化' : status.state === 'generating' ? '生成中' : '生成被拒绝'}</div>{status.graph ? <GraphSurface snapshot={status.graph} /> : <div className="graph-empty"><Workflow size={25} /><b>{status.state === 'rejected' ? '保留上一次审阅图' : '暂无审阅图'}</b><span>{status.diagnostics[0]?.message ?? '生成后将在此处显示只读工作流结构。'}</span></div>}{status.diagnostics.length ? <ul className="graph-diagnostics">{status.diagnostics.map((diagnostic, index) => <li key={`${diagnostic.message}-${index}`}>{diagnostic.message}</li>)}</ul> : null}</section>;
}
