...
function main() {
  ...
  const tool = payload.tool_name || payload.tool || payload.__tool || 'Unknown';
  ...
  const decision = {
    decision: /(?:rm\s+-rf|git\s+reset\s+--hard|git\s+push\s+--force|curl\s+.*\|\s*(?:bash|sh)|wget\s+.*\|\s*(?:bash|sh))/i.test(command)
      ...
      : /(?:secret|token|password|\.env|private[_-]?key)/i.test(command)
        ? 'deny'
        : 'allow',
    action: /deny/.test(command) ? 'block' : 'continue',
    dimension: 'permission',
    host: 'claude-code',
    tool,
    command,
    reason: 'claude-code adapter: local guard evaluation',
  };
