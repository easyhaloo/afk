...
import { createAgentProvider, resolveAgentProviderName } from '../lib/agents';
  ...
  program
    ...
    .action(async options => {
      try {
        ...
        const agentProvider = createAgentProvider(resolveAgentProviderName(options.agent ?? config.agentDefault));
        ...
        const runner = new QARunner(providers, config, { executionMode: options.mode, agentProvider });
