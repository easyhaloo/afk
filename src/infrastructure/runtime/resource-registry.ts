...
export interface AfkResourceMetadata {
  [key: string]: unknown;
}
...
function rowToRecord(row: ResourceRow): AfkResourceRecord {
  let metadata: AfkResourceMetadata = {};
  try {
    const parsed = JSON.parse(row.metadata_json) as unknown;
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) metadata = parsed as AfkResourceMetadata;
  } catch {
