...
export interface IsolateConnectionInfo {
  isolateName: string;
  worktreeRoot: string;
  services: IsolateServiceInfo[];
}

export interface IsolateServiceInfo {
  name: string;
  host: string;
  port: number;
...
export class IsolateManager {
  ...
  /**
   * Start isolate: parse compose → allocate ports → write .env → up → health check.
   * Returns connection info for the agent to use.
   */
  async up(): Promise<IsolateConnectionInfo> {
    if (!this.available()) {
  ...
  async discard(): Promise<void> {
    ...
    // Remove .env if it was created by us (has ISOLATE_ prefix marker)
    const envFile = join(this.worktreeRoot, '.env');
    if (existsSync(envFile)) {
      try {
        const content = await fs.readFile(envFile, 'utf-8');
        if (content.includes('# AFK isolate — do not edit')) {
          await fs.rm(envFile);
        }
      } catch { /* */ }
    }
   ...
   */
  getConnectionInfo(): IsolateConnectionInfo {
    const services: IsolateServiceInfo[] = this.services.map(svc => ({
      name: svc.name,
      host: '127.0.0.1',
      port: svc.hostPort + (this.offset ?? 0),
      containerPort: svc.containerPort,
      envVar: `${svc.name.toUpperCase()}_ISOLATE_PORT=${svc.hostPort + (this.offset ?? 0)}`,
    }));

    return {
      isolateName: this.projectHash,
   ...
   * This is a best-effort: the port offset is still applied via the ISOLATE_*
  ...
  private async writeEnv(): Promise<void> {
    ...
    const lines: string[] = [
      '# AFK isolate — do not edit',
      `# Generated by afk isolate (offset: ${this.offset})`,
      '',
    ];
    ...
    for (const svc of this.services) {
      ...
      lines.push(`${name}_ISOLATE_PORT=${offsetPort}`);
      lines.push(`ISOLATE_${name}_PORT=${offsetPort}`);
    }
    ...
    for (const svc of this.services) {
      ...
      const key = `ISOLATE_PORT_${svc.hostPort}`;
 ...
 */
export async function isolateGc(): Promise<number> {
