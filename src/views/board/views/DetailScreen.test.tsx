...
describe('DetailScreen', () => {
  ...
  it('renders task runtime fields in canonical order without owning a footer', () => {
    const task = { iid: 7, title: 'Run task', status: 'active' as const, branch: 'main', session: 'afk-7', progress: '50%', worktree: '/tmp/afk-7' };
    const output = renderToString(<DetailScreen item={task} view="tasks" height={12} width={80} />);

    expect(output.indexOf('session · afk-7')).toBeLessThan(output.indexOf('status · active'));
    expect(output.indexOf('status · active')).toBeLessThan(output.indexOf('worktree · /tmp/afk-7'));
